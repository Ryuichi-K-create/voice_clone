"""
Qwen3-TTS ファインチューニング用 Dataset

公式 finetuning/dataset.py をベースに作成。
"""
    ...
    def _normalize_audio_inputs(
        ...
        for a in items:
            if isinstance(a, str):
                ...
                raise ValueError("numpy配列入力にはサンプリングレートが必要です: (audio, sr)")
            else:
                raise TypeError(f"未対応の音声入力型: {type(a)}")
        return out
    ...
    def collate_fn(self, batch: list[dict]) -> dict:
        ...
        for i, data in enumerate(batch):
            ...
            input_ids[i, 3:8, 1] = torch.tensor([
                ...
                0,  # speaker embedding用
                self.config.talker_config.codec_pad_id,
            ])
            input_ids[
                i, 8 : 8 + text_ids_len - 3, 1
            ] = self.config.talker_config.codec_pad_id
            ...
            input_ids[
                i, 8 + text_ids_len - 1 + codec_ids_len, 1
            ...
            codec_0_labels[
                i, 8 + text_ids_len - 1 + codec_ids_len
            ] = self.config.talker_config.codec_eos_token_id
            ...
            codec_embedding_mask[i, 6] = False  # speaker embedding用
